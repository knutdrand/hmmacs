...
def run(bedgraphs, model):
    X, run_lengths, lengths, chroms = from_bedgraphs(bedgraphs)
    print(X)
    print(lengths)
    model.fit(X, run_lengths, lengths)
    states = model.predict(X, run_lengths, lengths)
...
def from_bedgraphs(bedgraphs):
    ...
    for chrom, bedgraph in bedgraphs:
        ...
        print(bedgraph._values)
        xs.append(bedgraph._values)
        assert bedgraph._values.size, "Empty chromosome"
        run_lengths.append(np.diff(np.append(bedgraph._indices, N)))
        chroms.append(chrom)
        lengths.append(xs[-1].size)
    assert len(xs), "Empty bedgraphs file"
    print(xs)
    X = np.concatenate(xs)
    run_lengths = np.concatenate(run_lengths)
    return X.reshape((-1, 1)), run_lengths.reshape((-1, 1)), np.array(lengths), chroms
